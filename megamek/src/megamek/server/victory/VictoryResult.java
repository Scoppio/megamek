...
public final class VictoryResult {
     ...
     * the victory result.
     *
     * @param twGame The current {@link TWGame} for which the victory needs to be processed
     * @return a list of reports generated by victory checking and by the actual processing of the victory
     */
    public List<Report> processVictory(TWGame twGame) {
        ...
        if (isVictory()) {
            ...
            if (wonPlayer != Player.PLAYER_NONE) {
                gatheredReports.add(Report.publicReport(7200).add(twGame.getPlayer(wonPlayer).getColorForPlayer()));
            }
            ...
            if (draw) {
                // multiple-won draw
                twGame.setVictoryPlayerId(Player.PLAYER_NONE);
                twGame.setVictoryTeam(Player.TEAM_NONE);
            } else {
                // nobody-won draw or single player win or single team win
                twGame.setVictoryPlayerId(wonPlayer);
                twGame.setVictoryTeam(wonTeam);
            }
        } else {
