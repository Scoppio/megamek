...
public class TeleMissileAttackAction extends AbstractAttackAction {
     ...
     * entities
     */
    private boolean checkPDConditions(IGame IGame, Targetable target) {
        advancedPD = IGame.getOptions().booleanOption(OptionsConstants.ADVAERORULES_STRATOPS_ADV_POINTDEF);
        if ((target == null)
     ...
     */
    public int calcCounterAV(IGame IGame, Targetable target) {
        if (!checkPDConditions(IGame, target)) {
            return 0;
        }
        int counterAV = 0;
        int amsAV = 0;
    ...
    public ToHitData toHit(TWGame twGame, Targetable target) {
